...
- `pkgs/`：第三方软件自打包（nixpkgs 没有的软件），`default.nix` 聚合导出（`{ pkgs }: { ... }`），由 `home/app.nix` 等处 `import ../pkgs { inherit pkgs; }` 引入安装；现有 `mazi51`（51mazi 小说写作软件，AppImage 经 `appimageTools.wrapType2` 打包，**GitHub release 资源域名在本网络 DNS 被污染，更新版本时需先经 v2rayA 代理 `nix store prefetch-file` 预取进 store**，否则 fetchurl 构建期下载失败）。
...
- Nix 文件：两空格缩进，函数参数用 `{ config, lib, pkgs, ... }:` 解构；注释保持**精简**——只为非显然的"为什么"写一行短注释，不逐条解释配置块用途（2026-07 起由高密度注释改为精简风格）。dotfiles（kdl/lua/sh 等）注释同此标准。**例外**：软件安装块（home.packages/systemPackages/flatpak packages/字体包等）在列表上方写分类清单注释，逐类列出装了什么及非显然用途（范例见 `home/app.nix`）。
