...
- `modules/home/`：共享 HM 配置（与用户身份无关，所有挂载用户自动获得；`home.stateVersion = "25.05"`），`default.nix` 汇总并导入 desktop/shell/app；另在 `default.nix` 中把 nixkits flake 的 `skills/` 目录逐个链接到 `~/.agents/skills/`（kimi-code 用户级技能目录，nix store 只读链接，`nix flake update` + rebuild 即更新技能）；**不显式设置 `home.username`/`home.homeDirectory`**——由 HM NixOS 模块按系统用户配置推断，多用户自动正确
  ...
  - `shell.nix`：zsh（vi 键位、别名、`nrs` = `nh os switch` 快捷别名）、fzf、yazi、neovim、nh（nix 命令助手，**首选系统管理命令**，每日自动清理：保留最近 3 个世代 + 7 天内的世代）。git 身份不在此（见 `home/<user>/`）。
...
- `home/<user>/`：每用户薄身份层——仅一个 `default.nix`（imports `../../modules/home` + `programs.git` 提交署名）；`host.nix` 的 `users` 清单中存在对应 `home/<user>/` 目录才会为该用户挂载 HM（自动获得 `modules/home/` 全部共享配置）。init.sh 的 `seed_home_dir` 会给新用户名从模板用户复制此目录。
- `dotfiles/`：应用配置文件，由 `modules/home/default.nix` 通过 `config.lib.file.mkOutOfStoreSymlink` 链接到 `~/.config/`（**指向仓库本身的活链接，不在 Nix store 中**）。
  - `niri/`：Niri 配置，按主题拆分为多个 `.kdl` 文件（`config.kdl` 为主入口，include `layout/animations/binds/windowrules/cursor/outputs/blur.kdl`）+ `scripts/`。**niri 是逐文件链接**（`~/.config/niri` 为真实目录）：`noctalia.kdl`（焦点环/边框等颜色）由 Noctalia 主题模板在运行时生成，不在仓库中；`config.kdl` 末尾已 include 它。`binds.kdl` 的键位布局复刻自 [shorin-arch-setup](https://github.com/SHORiN-KiWATA/shorin-arch-setup)（已适配 v5 `noctalia msg`、kitty→foot、firefox→brave）；`scripts/` 含 `portal-watcher.sh`（深浅色同步）、`screenshot-sound.sh`（截图快门声）、`screenshot-edit.sh`（satty 截图标注）、`niri-binds`（快捷键速查表）、`niri-force-kill-window`（强杀窗口）、`niri-pick`（窗口信息/取色）、`random-anime-wallpaper`（在线壁纸下载）。链接清单由 `modules/home/default.nix` 用 `builtins.readDir` 自动枚举（顶层常规文件 + `scripts/` 下常规文件），新增脚本或 .kdl 文件无需手工登记。
  ...
  - `fastfetch/`、`xsettingsd/`（图标主题为 `Adwaita-Matugen-B`，见下）、`rime/`（仅 `default.custom.yaml`，链接到 `~/.local/share/fcitx5/rime/`）、`Thunar/`（右键自定义动作 `uca.xml` + `accels.scm` 快捷键，配合 `~/Templates` 模板与 helpers.rc 的 foot）、`Templates/`（WPS 空白 Office 模板，经 `modules/home/desktop.nix` 链到 `~/Templates/`）。
  - `noctalia/`：Noctalia 自定义主题模板源 + 初始配置种子。**不链接**到 `~/.config/noctalia`（Noctalia 的运行时状态不纳入 git 活链接），而是由 `modules/home/default.nix` 的 `home.activation.noctaliaSeed` 在目标缺失时种子（详见该目录自带的 `README.md`）：
...
- `overlays/`：自定义 nixpkgs overlay。目录下每个 `.nix` 文件是一个 overlay（`final: prev: { ... }`），`default.nix` 自动聚合成列表，由 `flake.nix` 应用到 `nixpkgs.overlays`，新增文件无需登记（需 git 跟踪才可见）。现有：`wemeet.nix`（桌面文件 Exec 改走 wemeet-xwayland 包装，见 `modules/home/app.nix` 条目）、`fcitx5-rime.nix`（附加 rime-ice 词库）、`pkgs.nix`（把 `pkgs/` 自打包软件注入包集）。
- `libs/`：自定义函数库。目录下每个 `.nix` 文件形式为 `{ lib }: <函数或属性集>`，按文件名聚合（`libs/foo.nix` → `mylib.foo`），`mylib` 经 `specialArgs` 注入所有 NixOS 与 Home Manager 模块。现有：`regularFilesIn.nix`（列出目录常规文件名，`modules/home/default.nix` 用于枚举 niri 逐文件链接清单）。
- `pkgs/`：第三方软件自打包（nixpkgs 没有的软件），`default.nix` 聚合导出（`{ pkgs }: { ... }`），经 `overlays/pkgs.nix` 注入包集后直接用 `pkgs.<name>` 引用；现有 `mazi51`（51mazi 小说写作软件，AppImage 经 `appimageTools.wrapType2` 打包，**GitHub release 资源域名在本网络 DNS 被污染，更新版本时需先经 v2rayA 代理 `nix store prefetch-file` 预取进 store**，否则 fetchurl 构建期下载失败）、`dsh`（[DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) agent harness，官方 npm 包经 `buildNpmPackage` + `pkgs/dsh/` 自建 package-lock.json 打包——上游是 pnpm monorepo 无 npm lockfile；升级步骤见 `pkgs/dsh/default.nix` 头部注释：改版本 → 重新生成 lockfile → 重求 npmDepsHash）。
...
- Nix 文件：两空格缩进，函数参数用 `{ config, lib, pkgs, ... }:` 解构；注释保持**精简**——只为非显然的"为什么"写一行短注释，不逐条解释配置块用途（2026-07 起由高密度注释改为精简风格）。dotfiles（kdl/lua/sh 等）注释同此标准。**例外**：软件安装块（home.packages/systemPackages/flatpak packages/字体包等）在列表上方写分类清单注释，逐类列出装了什么及非显然用途（范例见 `modules/home/app.nix`）。
- 机器相关的可变性（cpu/gpu/userName 等）一律通过 `hosts/<host>/host.nix` 的参数读取，**不要硬编码**。
- 新增 GUI/CLI 软件包：加到 `modules/home/app.nix` 或 `modules/home/shell.nix`/`modules/home/desktop.nix`（按用途分类）；系统级组件加到 `modules/nixos/` 下对应主题文件（无对应主题时新建模块并在 `modules/nixos/default.nix` 登记）。
- 新增应用配置目录：放入 `dotfiles/<name>/`，并在 `modules/home/default.nix` 的 `configs` 表中登记，使其链接到 `~/.config/<name>`。
...
5. **kitsfmt 0.5.0 已知 bug**：会把 `++`（列表拼接）错误格式化成 `+`，破坏求值（2026-07 三次全仓库格式化分别踩中 4 处、3 处和 4 处，均已手工修复），两种模式下都会触发；其 best-practices 重写还会把 `with pkgs; [ ... ]` 改写成 `builtins.attrValues { inherit (pkgs) ...; }`（丢失列表内逐条注释、包顺序被字母排序）。本仓库**接受 attrValues 形式**（2026-07 第二次格式化起），包列表的逐条用途注释改写为列表上方的块注释（范例见 `modules/home/desktop.nix` 的 `home.packages`）。每次运行 kitsfmt 后必须 `nixos-rebuild dry-build` 验证并检查 `++`→`+` 损坏。另一已知 bug：`overrideAttrs (old: { ... })`（含多行字符串）会被重排成缩进混乱的形式，`overlays/wemeet.nix` 的 wemeet 覆盖块因此保持手工格式，不追求通过 `kitsfmt -c`。
